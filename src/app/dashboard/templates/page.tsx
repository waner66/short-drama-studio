...
interface MyTemplate {
  ...
  status: 'DRAFT' | 'REVIEWING' | 'PUBLISHED' | 'DELISTED';
  salesCount: number; avgRating: number; revenue: number;
}
...
const mockTemplates: MyTemplate[] = [
  { id: '1', title: '霸道总裁爱上我 · 短剧模板', description: '经典都市甜宠短剧模板，含角色设定+3幕剧本+分镜脚本', price: 29.9, status: 'PUBLISHED', salesCount: 234, avgRating: 4.8, revenue: 4896.6 },
  { id: '2', title: '穿越古代当首富 · 完整模板', description: '穿越经商双线并行，5幕结构，3角色+配音参考', price: 49.9, status: 'REVIEWING', salesCount: 0, avgRating: 0, revenue: 0 },
];
...
export default function MyTemplatesPage() {
  ...
  const totalRevenue = mockTemplates.reduce((s, t) => s + t.revenue, 0);
  const totalSales = mockTemplates.reduce((s, t) => s + t.salesCount, 0);
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs px-2 py-0.5 rounded-full bg-purple-500/20 text-purple-400">{mockTemplates.length} 个模板</span>
        </div>
        <GradientBtn onClick={() => setShowPublish(true)}>+ 发布新模板</GradientBtn>
      </div>
      ...
      {mockTemplates.length === 0 ? (
        <GlassCard>
          <EmptyState title="还没有模板" description="将你的项目发布为模板出售" actions={<GradientBtn onClick={() => setShowPublish(true)}>发布第一个模板</GradientBtn>} />
        </GlassCard>
      ) : (
              ...
              <div className="flex items-center gap-4 flex-wrap">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className={`flex-shrink-0 text-xs px-2 py-0.5 rounded-full ${statusCfg[tpl.status].cls}`}>
                      {statusCfg[tpl.status].label}
                    </span>
                  </div>
                  <p className="text-sm text-gray-500 truncate">{tpl.description}</p>
                </div>

                {/* 数据 */}
                <div className="text-right flex-shrink-0">
                  <div className="text-lg font-bold text-red-400">¥{tpl.price}</div>
                  <div className="text-xs text-gray-500 flex items-center gap-2 mt-0.5">
                    <span>{tpl.salesCount} 销量</span>
                    {tpl.avgRating > 0 && (
          ...
          <div className="w-full max-w-lg bg-[#141428] border border-white/10 rounded-2xl p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <GradientBtn onClick={confirmPublish} loading={publishing}>
                  {publishing ? '发布中...' : '确认发布'}
                </GradientBtn>
              </div>
            </div>
